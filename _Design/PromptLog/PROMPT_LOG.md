...
**카테고리:** DEBUG
**요청:**
PrisonerExpandZone이 Player 입장을 감지못하는것같은데 로직 확인해봐
...
**카테고리:** CODE
**요청:**
오케이 그렇게 진행하자. (UpgradeProgressUI 구현 승인)
...
**카테고리:** CODE
**요청:**
DOTween 연출은 지금 추가해두자. 모든 Zone공통인가?
...
**카테고리:** CODE
**요청:**
UpgradeZone은 단순 트리거가 아니라 금액을 지불해야 업그레이드 기능들을 사용할수있잖아? 관련한 로직까진 안되고 지금단순 enter로만 처리되는것같은데 맞아?
...
**카테고리:** CODE
**요청:**
이번세션에서 진행한 스크립트 작업이랑, 해당 클래스들에 맞게 진행되야하는 에디터작업 전체적으로 정리해줄래? 폴더 정리때문에 아직 스크립트들 내용 검토를 못했어
...
**카테고리:** DEBUG
**요청:**
폴더 정리하면서 씬에 배치해둔 오브젝트들의 스크립트 연결이 다 끊겼는데, 다 다시 세팅해야되는거야..?
